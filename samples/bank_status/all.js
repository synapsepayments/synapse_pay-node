...
SynapsePay.User.login("3ac38d63db58466982fe6f871c48f1", "TestTest123$").then(function(client) {
  ...
  // With a promise
  client.bankStatuses.all().then(function(bankStatuses) {
    _.each(bankStatuses.data, function(bankStatus) {
      console.log(bankStatus);
    });
  }).catch(function(err) {
    console.log("Got an error in BankStatusEndpoint#all.");
    console.log(err);
  });

  // With a callback
  client.bankStatuses.all(function(bankStatuses, err) {
    if(err) {
      console.log("Got an error in BankStatusEndpoint#all.");
      console.log(err);
    } else {
