...
SynapsePay.User.login("3ac38d63db58466982fe6f871c48f1", "TestTest123$").then(function(client) {
  ...
  // With a promise
  client.bankMfaQuestions.answer("access_token", "Bank of America", "test_answer").then(function(banks) {
    _.each(banks.data, function(bank) {
      console.log(bank);
    });
  }).catch(function(err) {
    console.log("Got an error in BankMfaQuestionsEndpoint#answer.");
    console.log(err);
  });

  // With a callback
  client.bankMfaQuestions.answer("access_token", "Bank of America", "test_answer", function(banks, err) {
    if(err) {
      console.log("Got an error in BankMfaQuestionsEndpoint#answer.");
      console.log(err);
    } else {
